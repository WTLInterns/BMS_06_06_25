...
public class VendorVehicleController {
    ...
    public ResponseEntity<?> createVehicle(@PathVariable Long vendorId, @ModelAttribute VendorVehicle vendorVehicle,
                                         ...
                                         @RequestParam(value = "cabSideImageFile", required = false) MultipartFile cabSideImageFile) throws IOException {
        Vendor vendor = vendorRepository.findById(vendorId).orElse(null);
        if (vendor == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Invalid vendor ID");
        }
        vendorVehicle.setVendor(vendor);
        return ResponseEntity.status(HttpStatus.CREATED).body(vendorVehicleService.createVehicle(vendorVehicle, rCImageFile, vehicleNoImageFile, insuranceImageFile, permitImageFile, authorizationImageFile, cabNoPlateImageFile, cabImageFile, cabFrontImageFile, cabBackImageFile, cabSideImageFile));
    }
    ...
    public ResponseEntity<VendorVehicle> updateVehicle(@PathVariable Integer vehicleId, @ModelAttribute VendorVehicle vehicleDetails,
                                                     @RequestParam(value = "rCImageFile", required = false) MultipartFile rCImageFile,
                                                     @RequestParam(value = "vehicleNoImageFile", required = false) MultipartFile vehicleNoImageFile,
                                                     @RequestParam(value = "insuranceImageFile", required = false) MultipartFile insuranceImageFile,
                                                     @RequestParam(value = "permitImageFile", required = false) MultipartFile permitImageFile,
                                                     ...
                                                     @RequestParam(value = "cabBackImageFile", required = false) MultipartFile cabBackImageFile,
                                                     @RequestParam(value = "cabSideImageFile", required = false) MultipartFile cabSideImageFile) throws IOException {
        return ResponseEntity.ok(vendorVehicleService.updateVehicle(vehicleId, vehicleDetails, rCImageFile, vehicleNoImageFile, insuranceImageFile, permitImageFile, authorizationImageFile, cabNoPlateImageFile, cabImageFile, cabFrontImageFile, cabBackImageFile, cabSideImageFile));
    }
